...
@pytest.mark.anyio
async def test_get_user_with_JWT(registered_user: dict):
    token = security.create_access_token(registered_user["email"])

    user = await security.get_user_with_JWT(token)

    assert user.email == registered_user["email"]

@pytest.mark.anyio
async def test_get_user_with_invalid_JWT():
    with pytest.raises(security.HTTPException):
        await security.get_user_with_JWT("Invalid Token")